...
impl<'a> HistoryRepository<'a> {
    ...
    pub async fn create(
        &self, user_id: i64, note_id: i64, version_number: i64, operation: &str,title: &str, content: &str, 
    ) -> Result<u64> {
        create::create(
            self.conn, user_id, note_id, version_number, operation, &title, &content
        ).await
    }
    
    pub async fn get_one(
        &self, user_id: i64, note_id: i64, version_number: i64
    ) -> Result<NoteHistory> {
    ...
    pub async fn restore(
            &self,  user_id: i64, note_id: i64, new_title: &str, new_content: &str
        ) -> Result<u64> {
